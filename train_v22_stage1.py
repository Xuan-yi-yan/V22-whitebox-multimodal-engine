...
for _vch in "苹果太阳低中高音":  # V22: 视觉+声学标签字并入词表(否则标签变<unk>)
    if _vch not in c2i: c2i[_vch] = len(c2i) + 1
c2i['<unk>'] = 0; c2i['<pad>'] = -1
...
class ABC_StageD(nn.Module):
    """D: 关系审查 — 关系即属性(P3逻辑关系槽42-52), ABC审查其自洽性, 不引入GNN.
       ct(384) → 关系表征 rd(384) + 关系类logits(11: 因果/转折/并列/条件/递进/让步/选择/承接/总结/举例/目的)"""
    def __init__(self, content_dim=384, n_rel=11):
        super().__init__()
        self.encoder = nn.Sequential(nn.Linear(content_dim, 384), nn.GELU(), nn.Linear(384, 384), nn.GELU())
        self.rel_head = nn.Linear(384, n_rel)
        for m in self.modules():
            if isinstance(m, nn.Linear): nn.init.xavier_uniform_(m.weight, gain=0.5); nn.init.zeros_(m.bias)
    def forward(self, ct):
        rd = self.encoder(ct)
        return self.rel_head(rd), F.normalize(rd, dim=-1)  # rd球面归一化: 控数值范围+梯度冲击

class ABC_StageE(nn.Module):
    """E: 记忆读写头 — 算好的表征(sent_vec→me)写入记忆库(可落文件), 读时检索
       最相似历史me, E层球面融合. 全量融合(0.1), 数值由球面归一化控, 不冲现有."""
    def __init__(self, dim=384, mem_cap=2000):
        super().__init__()
        self.net = nn.Sequential(nn.Linear(dim, dim), nn.GELU(), nn.Linear(dim, dim))
        for m in self.net:
            if isinstance(m, nn.Linear): nn.init.xavier_uniform_(m.weight, gain=0.5); nn.init.zeros_(m.bias)
        self.mem_keys = None   # [N,256] 历史sent_vec (检索key)
        self.mem_vals = None   # [N,dim] 历史me       (记忆内容)
        self.mem_cap = mem_cap
    def retrieve(self, sv):    # sv:[bs,256] → mem_recall:[bs,dim] or None
        if self.mem_keys is None or self.mem_keys.shape[0] == 0: return None
        sim = F.normalize(sv, dim=-1) @ F.normalize(self.mem_keys, dim=-1).T  # [bs,N] 余弦
        return self.mem_vals[sim.argmax(dim=1)]                              # 各取最相似top1
    def write(self, sv, me):   # 写头: 存 (sent_vec, me), FIFO上限
        k, v = sv.detach(), me.detach()
        self.mem_keys = k if self.mem_keys is None else torch.cat([self.mem_keys, k])[-self.mem_cap:]
        self.mem_vals = v if self.mem_vals is None else torch.cat([self.mem_vals, v])[-self.mem_cap:]
    def forward(self, rd, sv=None):
        rec = self.retrieve(sv) if sv is not None else None
        me = F.normalize(self.net(rd), dim=-1)                               # 球面
        if rec is not None: me = F.normalize(me + rec * 0.1, dim=-1)         # 记忆召回融合
        if sv is not None and self.training: self.write(sv, me)             # 写头(训练时)
        return me
    def save_mem(self, path):  # 写文件
        if self.mem_keys is not None:
            torch.save({'k': self.mem_keys.cpu(), 'v': self.mem_vals.cpu()}, path)
    def load_mem(self, path):  # 读文件
        if os.path.exists(path):
            d = torch.load(path); self.mem_keys = d['k'].to(DEVICE); self.mem_vals = d['v'].to(DEVICE)

abcA = ABC_StageA().to(DEVICE)
abcB = ABC_StageB().to(DEVICE)
abcC = ABC_StageC().to(DEVICE)
abcD = ABC_StageD().to(DEVICE)
abcE = ABC_StageE().to(DEVICE)
abcE.load_mem("C:/ai/P1_char_word/checkpoints/V22_mem.pt")  # E记忆库: 读历史(首次为空)
...
rd_to_gate = nn.Linear(384, 128, bias=False).to(DEVICE)   # V22: D关系表征→gate (第4路)
me_to_sent = nn.Linear(384, 256, bias=False).to(DEVICE)   # V22: E记忆→P6加法注入
nn.init.xavier_uniform_(me_to_sent.weight, gain=0.5)      # 全量注入, 数值由me球面归一化控制
...
opt_abc = torch.optim.Adam(list(abcA.parameters())+list(abcB.parameters())+list(abcC.parameters())+list(abcD.parameters())+list(abcE.parameters()), lr=slr_abc.lr)
opt_gate= torch.optim.Adam(list(explore.parameters())+list(meta.parameters())+list(abc_to_gate.parameters())+list(sent_to_gate.parameters())+list(gate_base_proj.parameters())+list(abc_to_sent.parameters())+list(rd_to_gate.parameters())+list(me_to_sent.parameters()), lr=slr_gate.lr)
...
def save_best(sig=None, frame=None):
    abcE.save_mem("C:/ai/P1_char_word/checkpoints/V22_mem.pt")  # E记忆库: 写文件(与权重同存)
...
class VisualEmbed(nn.Module):
    """16x16像素 → 768 RGB → 256d, 正交init (对应 char_embed)"""
    def __init__(self, grid=16, out_dim=256):
        super().__init__(); self.grid = grid
        self.proj = nn.Linear(grid*grid*3, out_dim, bias=False)
        nn.init.orthogonal_(self.proj.weight)
    def forward(self, px): return self.proj(px)

vembed = VisualEmbed().to(DEVICE)
opt_vembed = torch.optim.Adam(vembed.parameters(), lr=args.lr)

def _img_pixels(img, grid=16):
    s = cv2.resize(img, (grid, grid), interpolation=cv2.INTER_AREA)
    return cv2.cvtColor(s, cv2.COLOR_BGR2RGB).astype('float32').flatten() / 255.0
...
for _d in _img_dirs:
    for _f in glob.glob(os.path.join(_d, "*.bmp")):
        ...
        for _k, _lb in _VLABEL.items():
            if _nm.startswith(_k):
                _im = cv2.imread(_f)
                if _im is not None: _vpairs.append((_im, _lb))
                break
w(f"[视觉] 图文对: {len(_vpairs)} (apple→苹果, sun→太阳)")

def vision_step():
    """图→文: 视觉token(像素球面256 + P3Vis 384)→引擎→P6解码标签 + 图↔文余弦对比"""
    if not _vpairs: return None
    import random as _r
    bt = _r.sample(_vpairs, min(8, len(_vpairs)))
    v_embs, v_attrs, lab_ids, t_embs = [], [], [], []
    for img, lab in bt:
        px = torch.tensor(_img_pixels(img), device=DEVICE, dtype=torch.float32)
        v_embs.append(F.normalize(vembed(px), dim=-1))                              # 像素权重 球面256
        va = torch.zeros(384, device=DEVICE)
        va[128:384] = torch.tensor(_p3vis.process_image(img), dtype=torch.float32, device=DEVICE)
        v_attrs.append(va)
        lids = [c2i.get(c, 0) for c in lab]; lab_ids.append(lids)
        t_embs.append(F.normalize(char_embed(torch.tensor(lids, device=DEVICE)).mean(0), dim=-1))  # 文权重 球面256
    bs = len(bt); vocab = char_embed.weight.detach()
    v_emb_b = torch.stack(v_embs)                                                   # [bs,256]
    v_rich = torch.cat([v_emb_b.unsqueeze(1), torch.stack(v_attrs).unsqueeze(1)], dim=-1)  # [bs,1,640]
    wo, sv, _, _, _ = p7.forward_batch(p7_input_proj(v_rich), vocab.unsqueeze(0).expand(bs, -1, -1),
                                       [1]*bs, [vocab.shape[0]]*bs, last_loss=0.0)
    pred = p6(sv, char_embed.weight)                                                # [bs, max_words, vocab]
    ce = 0.0
    for i in range(bs):
        nB = min(len(lab_ids[i]), pred.shape[1])
        ce = ce + F.cross_entropy(pred[i, :nB, :], torch.tensor(lab_ids[i][:nB], device=DEVICE))
    ce = ce / bs
    cos_loss = (1 - F.cosine_similarity(v_emb_b, torch.stack(t_embs), dim=-1)).mean()  # 图↔文 余弦(硬编码)
    return ce + cos_loss

# ═══════════════════ V22 声学: 频谱→文 (FFT最底层, 球面, 模拟数据) ═══════════════════
class AudioEmbed(nn.Module):
    """FFT幅度谱(最底层, 无梅尔滤波) → 256d 球面 (对应 VisualEmbed/char_embed)"""
    def __init__(self, n_freq=512, out_dim=256):
        super().__init__()
        self.proj = nn.Linear(n_freq, out_dim, bias=False)
        nn.init.orthogonal_(self.proj.weight)
    def forward(self, fft): return self.proj(fft)

aembed = AudioEmbed().to(DEVICE)
opt_aembed = torch.optim.Adam(aembed.parameters(), lr=args.lr)
_ALABEL = {"low": "低音", "mid": "中音", "high": "高音"}

def _sim_fft(kind, n_freq=512):
    """模拟一串FFT幅度谱: 底噪 + 某频段能量集中 (低/中/高音)"""
    q = n_freq // 4
    fft = torch.rand(n_freq) * 0.1
    if kind == "low":   fft[:q] += torch.rand(q) * 2.0
    elif kind == "mid": fft[q:2*q] += torch.rand(q) * 2.0
    else:               fft[3*q:] += torch.rand(q) * 2.0
    return fft

def audio_step():
    """声→文: 模拟FFT → AudioEmbed球面 → 引擎 → P6解码标签 + 声↔文余弦"""
    import random as _r
    kinds = [_r.choice(list(_ALABEL)) for _ in range(8)]
    a_embs, lab_ids, t_embs = [], [], []
    for k in kinds:
        fft = _sim_fft(k).to(DEVICE)
        a_embs.append(F.normalize(aembed(fft), dim=-1))                            # 声学权重 球面256
        lab = _ALABEL[k]; lids = [c2i.get(c, 0) for c in lab]; lab_ids.append(lids)
        t_embs.append(F.normalize(char_embed(torch.tensor(lids, device=DEVICE)).mean(0), dim=-1))
    bs = len(kinds); vocab = char_embed.weight.detach()
    a_emb_b = torch.stack(a_embs)                                                  # [bs,256]
    a_rich = torch.cat([a_emb_b.unsqueeze(1), torch.zeros(bs, 1, 384, device=DEVICE)], dim=-1)  # attr暂0(P3Aud待落地)
    wo, sv, _, _, _ = p7.forward_batch(p7_input_proj(a_rich), vocab.unsqueeze(0).expand(bs, -1, -1),
                                       [1]*bs, [vocab.shape[0]]*bs, last_loss=0.0)
    pred = p6(sv, char_embed.weight)
    ce = 0.0
    for i in range(bs):
        nB = min(len(lab_ids[i]), pred.shape[1])
        ce = ce + F.cross_entropy(pred[i, :nB, :], torch.tensor(lab_ids[i][:nB], device=DEVICE))
    ce = ce / bs
    cos_loss = (1 - F.cosine_similarity(a_emb_b, torch.stack(t_embs), dim=-1)).mean()  # 声↔文 余弦
    return ce + cos_loss
...
for ep in range(1, args.epochs+1):
    ...
    while remaining:
        ...
        rel_logits, rd = abcD(ct)                      # D: 关系审查(P3关系槽自洽性)
        me = abcE(rd, sv_batch.detach())               # E: 记忆读写头(sv检索历史me→球面融合+写回)
        ...
        with torch.amp.autocast('cuda', enabled=args.amp):
            ...
            gate_input = torch.cat([gate_base_proj(abc_conf), aux_scale*abc_to_gate(ac_batch), aux_scale*sent_to_gate(sv_batch)], dim=-1)  # V22: 先稳住, 待gate分头重构
            gates = meta(explore(gate_input))
            # V21.1: abc_prime加法注入sent (保sv_batch主导, abc_prime渐进参与)
            sv_rich = sv_batch + abc_to_sent(abc_prime_out) * 0.1 + me_to_sent(me) * 0.1  # V22: E记忆全量注入(me已球面归一化, 数值受控)
        ...
        # loss_D: 关系审查 (P3逻辑关系槽42-52监督, 无关系激活→类0)
        rel_tgt = attr_b[:, :, 42:53].sum(dim=1).argmax(dim=1)  # [bs] 主关系类0-10
        loss_D_c = F.cross_entropy(rel_logits, rel_tgt)
        ...
        batch_loss = ce_loss + aux_scale*(temps[1]*loss_A_c + temps[2]*loss_B_c + temps[3]*loss_C_c + 0.01*loss_D_c) + temps[0]*gate_loss  # V22: loss_D冷启动固定0.01, 不进master动态防震荡
...
# ════════════════════════ 保存 ════════════════════════
# ═══════════════════ V22 视觉训练: 图→文 (文本训完后, 硬编码余弦) ═══════════════════
if _vpairs:
    w(f"\n[视觉训练] 图→文 {len(_vpairs)}对 × 30轮 (像素球面 + 余弦对比)")
    v_opts = [opt_p7_in, opt_p7, opt_p6, opt_vembed]
    for vep in range(1, 31):
        for o in v_opts: o.zero_grad()
        vl = vision_step()
        if vl is not None:
            vl.backward()
            for o in v_opts: o.step()
            if vep % 5 == 0: w(f"  视觉E{vep} loss={vl.item():.4f}")
    # 展示图→文预测
    import random as _r
    for img, lab in _r.sample(_vpairs, min(4, len(_vpairs))):
        with torch.no_grad():
            px = torch.tensor(_img_pixels(img), device=DEVICE, dtype=torch.float32)
            ve = F.normalize(vembed(px), dim=-1)
            va = torch.zeros(384, device=DEVICE)
            va[128:384] = torch.tensor(_p3vis.process_image(img), dtype=torch.float32, device=DEVICE)
            vr = torch.cat([ve, va]).unsqueeze(0).unsqueeze(0)                # [1,1,640]
            vocab = char_embed.weight
            _, sv, _, _, _ = p7.forward_batch(p7_input_proj(vr), vocab.unsqueeze(0), [1], [vocab.shape[0]], last_loss=0.0)
            pr = p6(sv, char_embed.weight)[0].argmax(-1)[:len(lab)]
            w(f"  图({lab}) → 预测: {''.join(i2c.get(int(i), '?') for i in pr)}")

# ═══════════════════ V22 声学训练: 频谱→文 (模拟FFT, 硬编码余弦) ═══════════════════
w(f"\n[声学训练] 模拟FFT→文 (低/中/高音) × 30轮 (FFT球面 + 余弦对比)")
au_opts = [opt_p7_in, opt_p7, opt_p6, opt_aembed]
for aep in range(1, 31):
    for o in au_opts: o.zero_grad()
    al = audio_step()
    al.backward()
    for o in au_opts: o.step()
    if aep % 5 == 0: w(f"  声学E{aep} loss={al.item():.4f}")
# 展示 声→文预测
for _k in ["low", "mid", "high"]:
    with torch.no_grad():
        fft = _sim_fft(_k).to(DEVICE)
        ae = F.normalize(aembed(fft), dim=-1)
        ar = torch.cat([ae, torch.zeros(384, device=DEVICE)]).unsqueeze(0).unsqueeze(0)   # [1,1,640]
        vocab = char_embed.weight
        _, sv, _, _, _ = p7.forward_batch(p7_input_proj(ar), vocab.unsqueeze(0), [1], [vocab.shape[0]], last_loss=0.0)
        pr = p6(sv, char_embed.weight)[0].argmax(-1)[:2]
        w(f"  声({_ALABEL[_k]}) → 预测: {''.join(i2c.get(int(i), '?') for i in pr)}")
...
w(f"  └──────────────────────────────────")
w(f"  ┌─ Gate状态 ───────────────────────")
w(f"  │ explore: signal_mean={explore(torch.randn(384,device=DEVICE)).mean().item():.3f}")
